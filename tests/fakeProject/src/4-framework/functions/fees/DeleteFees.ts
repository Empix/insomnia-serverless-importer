...
import { DeleteFeesOperator } from '@controller/operations/fees/DeleteFeesOperator'
import { InputDeleteFees } from '@controller/serializers/fees/DeleteFeesSerializer'
...
const main = httpHandler(async (event: APIGatewayProxyEvent, context: Context) => {
  ...
  const operator = container.get(DeleteFeesOperator)
  const input = new InputDeleteFees(event.pathParameters as Object)
